...
const Home = () => {
  ...
  const handleSearch = (query: string) => {
    let filteredProducts = [...products]; // Sao chép danh sách gốc
    ...
    // Tìm kiếm theo tên sản phẩm nếu có query
    if (query) {
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View className="w-[48%] mt-2 bg-white rounded-2xl p-2 flex justify-between">
              ...
              >
                <Image source={{ uri: item.image_url }} className="w-full h-32 rounded-2xl" />
